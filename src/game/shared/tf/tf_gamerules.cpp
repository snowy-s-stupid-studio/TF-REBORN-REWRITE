...
ConVar tf_mvm_forceversus("tf_mvm_forcebwr", "0", FCVAR_REPLICATED | FCVAR_NOTIFY, "Enable be with robots in MvM");
...
ConVar tf_gamemode_domination("tf_gamemode_domination", "0", FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DEVELOPMENTONLY);
...
Vector g_TFClassViewVectors[TF_CLASS_COUNT /* incase we need to do more classes, we do this also previsouly 11 */] =
{
	...
	Vector( 0, 0, 75 ),		// TF_CLASS_LOBSTER,
...
void CTFGameRules::ClientSettingsChanged( CBasePlayer *pPlayer )
{
	...
	// keep track of their cl_flipviewmodels value
	pTFPlayer->SetFlipViewModels( Q_atoi( engine->GetClientConVarValue( pPlayer->entindex(), "cl_flipviewmodels" ) ) > 0 );
...
int CTFGameRules::GetTeamAssignmentOverride( CTFPlayer *pTFPlayer, int iDesiredTeam, bool bAutoBalance /*= false*/ )
{
	...
	if (IsMannVsMachineMode() && !tf_mvm_bwr.GetBool())
	{
